// osal/example/osal_port_freertos/dp_osal_port.h
//
...
//   set(DP_OSAL_PORT_DIR ${CMAKE_CURRENT_SOURCE_DIR}/path/to/osal_port_freertos)
#pragma once
...
// ═══════════════════════════════════════════════════════════════════════════════
// #define DP_OSAL_BACKEND_POSIX
#define DP_OSAL_BACKEND_CMSIS_OS
...
//
// DP_OSAL_PORT_THREAD_MIN_STACK_SIZE — minimum thread stack in bytes.
...
//
// DP_OSAL_PORT_THREAD_DEFAULT_PRIORITY — default thread priority.
...
// ═══════════════════════════════════════════════════════════════════════════════
#ifndef DP_OSAL_PORT_THREAD_MIN_STACK_SIZE
#define DP_OSAL_PORT_THREAD_MIN_STACK_SIZE configMINIMAL_STACK_SIZE
#endif

#ifndef DP_OSAL_PORT_THREAD_DEFAULT_PRIORITY
#define DP_OSAL_PORT_THREAD_DEFAULT_PRIORITY osPriorityNormal
#endif
...
#define DP_OSAL_PORT_THREAD_STACK_POINTER_FROM_ID(thread_id) (*reinterpret_cast<uint32_t *>(thread_id))
#else
...
//
// The function is declared 'inline' because dp_osal_port.h is included by
...
//
// DP_OSAL_TEST_ALL = 1  → enable every test component (recommended default)
// DP_OSAL_TEST_ALL = 0  → use the per-component flags below for fine control
//
...
//
// All components were enabled in the original configuration → DP_OSAL_TEST_ALL = 1.
// ═══════════════════════════════════════════════════════════════════════════════
#define DP_OSAL_TEST_ALL 1

#if !DP_OSAL_TEST_ALL
// Set each to 1 (run tests) or 0 (GTEST_SKIP).
#define DP_OSAL_TEST_THREAD_ENABLED 1              // all thread tests enabled
#define DP_OSAL_TEST_CHRONO_ENABLED 1              // all chrono tests enabled
#define DP_OSAL_TEST_MUTEX_ENABLED 1               // all mutex tests enabled
#define DP_OSAL_TEST_CONDITION_VARIABLE_ENABLED 1  // all condition variable tests enabled
#define DP_OSAL_TEST_LOCKGUARD_ENABLED 1           // all lockguard tests enabled
#define DP_OSAL_TEST_MEMORY_MANAGER_ENABLED \
    1                                    // all memory manager tests enabled
                                         // (GetAllocatedSize was 0 in old format;
                                         //  per-case control now requires GTEST_SKIP)
#define DP_OSAL_TEST_QUEUE_ENABLED 1        // all queue tests enabled
#define DP_OSAL_TEST_SEMAPHORE_ENABLED 1    // all semaphore tests enabled
#define DP_OSAL_TEST_THREAD_POOL_ENABLED 1  // all thread pool tests enabled
#define DP_OSAL_TEST_TIMER_ENABLED 1        // all timer tests enabled
#define DP_OSAL_TEST_RWLOCK_ENABLED \
    1                                 // all rwlock tests enabled
                                      // (GetReadLockCount=0, IsWriteLocked=0 in old
                                      //  format; per-case control now requires GTEST_SKIP)
#define DP_OSAL_TEST_SPINLOCK_ENABLED 1  // all spinlock tests enabled
#endif
